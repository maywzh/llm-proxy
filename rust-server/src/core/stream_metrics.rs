...
pub fn record_stream_metrics(stats: &StreamStats) {
    ...
    if stats.input_tokens > 0 || stats.output_tokens > 0 {
        metrics
            .token_usage
            .with_label_values(&[&stats.model, &stats.provider, "prompt", &stats.api_key_name])
        ...
        metrics
            .token_usage
            .with_label_values(&[&stats.model, &stats.provider, "total", &stats.api_key_name])
...
mod tests {
    ...
    fn test_stream_stats_new() {
        ...
        let stats = StreamStats::new("gpt-4", "OpenAI", "test-key", start);
